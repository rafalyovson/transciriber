...
function setProgressView(percent, stageMessage) {
  const normalized = Number.isFinite(percent) ? Math.max(0, Math.min(100, Math.floor(percent))) : 0
...
function renderFinalResult(response, requestedMode) {
  ...
  if (response.isComplete !== true) {
    const coverageText = typeof response.coverageRatio === 'number'
      ? ` Coverage: ${(response.coverageRatio * 100).toFixed(2)}%.`
      : ''
    showError(`Transcription is incomplete and was not saved as a successful run.${coverageText}`)
  ...
  setSubtitleArtifacts({
    ...
    violationCount: response.subtitleViolationCount ||
      response.subtitleQuality?.violations?.length || 0,
  ...
  const fallbackNotice = response.fallbackApplied
    ...
    : ''
  const warningNotice = Array.isArray(response.warnings) && response.warnings.length > 0
    ? ` ${response.warnings.join(' ')}`
    : ''
  const extractionNotice = response.audioExtracted
    ? ' Audio was extracted from MP4 before transcription.'
    : ''
  const chunkSummary = typeof response.totalChunks === 'number'
    ? ` Chunks: ${response.successfulChunks || 0}/${response.totalChunks} successful.`
    : ''
  const coverageSummary = typeof response.coverageRatio === 'number'
    ? ` Coverage: ${(response.coverageRatio * 100).toFixed(2)}%.`
    : ''
  ...
  const subtitleQualitySummary = response.subtitleQuality
    ...
    : ''
  const subtitleViolationCount = response.subtitleViolationCount ??
    response.subtitleQuality?.violations?.length ??
    0
  const subtitleWarnings = subtitleViolationCount > 0
  ...
  successMessage += extractionNotice + fallbackNotice + chunkSummary + coverageSummary +
    subtitleSummary + subtitlePathSummary + subtitleQualitySummary + subtitleWarnings +
    subtitleQualityReport + warningNotice
...
async function waitForTerminalSnapshot(jobId) {
  while (true) {
    ...
    if (!snapshot.success) {
      throw new Error(snapshot.error || 'Failed to retrieve transcription job status.')
    }

    if (snapshot.progress) {
      setProgressPercent(snapshot.progress.percent || 0)
      setCurrentStage(snapshot.progress.stage || 'queued')
      setChunkProgress(snapshot.progress.chunkIndex || 0, snapshot.progress.chunkTotal || 0)
      setProgressView(snapshot.progress.percent || 0, stageLabelFromEvent(snapshot.progress))
      DOMElements.progressMessageElement.textContent = stageLabelFromEvent(snapshot.progress)
    }
...
export function updateLanguageDisplay() {
  const selectedOption =
    DOMElements.languageSelect.options[DOMElements.languageSelect.selectedIndex]
...
export async function transcribeAudio() {
  ...
  DOMElements.progressMessageElement.textContent = 'Starting transcription job...'
  ...
  try {
    ...
    const startResponse = await Bridge.startTranscriptionJob(currentFile, options)
    if (!startResponse.success || !startResponse.jobId) {
      showError(`Failed to start job: ${startResponse.error || 'Unknown error'}`)
      DOMElements.progressMessageElement.textContent = 'Job failed.'
      DOMElements.outputSection.classList.add('hidden')
      return
    }
    ...
    await new Promise((resolve) => {
      ...
      source.onmessage = (messageEvent) => {
        try {
          ...
          if (event.partialText && (event.modeUsed === 'parts' || mode === 'parts')) {
